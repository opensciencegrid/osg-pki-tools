...
parser.add_argument("-f",  "--hostfile", action="store", dest="hostfile", required = True,
				  help="Filename with one hostname per line", metavar="HOSTFILE", default="hosts.txt")
parser.add_argument("-y",  "--country", action="store", dest="country", required = False,
				  help="Specify Country code for CSR (2 letter)", metavar="COUNTRY")
parser.add_argument("-s",  "--state", action="store", dest="state", required = False,
				  help="Specify State code for CSR (2 letter)", metavar="STATE")
parser.add_argument("-l",  "--locality", action="store", dest="locality", required = False,
				  help="Specify City for CSR", metavar="LOCALITY")
parser.add_argument("-o",  "--org", action="store", dest="org", required = False,
...
global country, state, locality, org, orgunit, hostname, domain, email, name, phone,outkeyfile
userprivkey = args.userprivkey
usercert = args.usercert
hostfile = args.hostfile

#######Optional arguments#####
if 'args.country' in locals():
    country = args.country
else:
    country=''

if args.state in locals():
    state = args.state
else:
    state=''

if args.locality in locals():
    locality = args.locality
else:
    locality=''

if args.org in locals():
    org = args.org
else:
    org=''

if args.orgunit in locals():
...
def connect_request(bulk_csr):
    ...
    conn = httplib.HTTPSConnection(host, key_file = outkeyfile, cert_file = usercert)
    try:
...
def connect_approve():    
    ...
    headers = {'Content-type': content_type,'User-Agent': 'OIMGridAPIClient/0.1 (OIM Grid API)'}
    conn = httplib.HTTPSConnection(host, key_file = outkeyfile, cert_file = usercert)
...
def connect_retrieve():
    ...
    certfile.close()
    os.system("openssl pkcs7 -print_certs -in "+filename+" -out "+filename)
    #os.remove(filename)
    os.chdir(cwd)
...
if __name__ == '__main__': 
	...
	#####Building Dictionary on optional Parameters######
