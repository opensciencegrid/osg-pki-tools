...
def parse_cli():
    ...
    usage = """%(prog)s [--debug] -u username -k pkey -c cert \\
    ...
    parser = argparse.ArgumentParser(
        add_help=False, usage=usage, description="Request and retrieve certificates from the  InCommon IGTF server CA."
    )
    ...
    optional.add_argument(
        ...
        action="store",
        dest="config_file",
        default="/etc/osg/pki/ca-issuer.conf",
    ...
    optional.add_argument(
        ...
        "--key-length",
        action="store",
        default=cert_utils.Csr.KEY_LENGTH,
        type=int,
        help="The key size to generate",
    )
    optional.add_argument(
        "--debug", action="store_true", dest="debug", default=False, help="Write debug output to stdout"
    )
    optional.add_argument(
        "-t",
        "--test",
...
class FilePathAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if not os.path.exists(values):
            raise OSError(f"Unable to locate the file at: {values}")

        try:
            open(values)
            setattr(namespace, self.dest, values)
        except OSError:
            raise OSError(f"Unable to read the file at: {values}")
    ...
    try:
        ...
        if response.status == 200:
            ...
            raise AuthenticationFailureException(
                response.status, "Connection failure to InCommon API. Check your authentication credentials."
            )
        else:
            print(
                prog + ": Connection failure to InCommon API. HTTP " + str(response.status) + " " + str(response.reason)
            )
...
def main():
    ...
    try:
        ...
        for host in set(hosts):
            ...
            csr_obj = cert_utils.Csr(
                common_name, output_dir=args.write_directory, altnames=sans, key_length=args.key_length
            )
