...
""" 
 This script allows a Grid Admin to approve, reject, or cancel a certificate
 request via the ID number that they receive via email and/or GOC ticket.
 
 Usage: ManageCertRequests.py [options]

 Options:
  -h, --help                    Show this help message and exit
  -pk PKEY, --pkey=PKEY         Requestor's private key (PEM Format)
  -ce CERT, --cert=CERT          Requestor's user certificate (PEM Format)
  -a ACTION, --action=ACTION    Action to take (reject, cancel, revoke)
...
parser.add_argument("-a",  "--action", action="store", dest="action", required = True,
				  help="Action to take (reject, cancel, revoke", metavar="ACTION")
parser.add_argument("-i",  "--id", action="store", dest="id", required = True,
				  help="Specify ID# of certificate request to act on", metavar="ID")
