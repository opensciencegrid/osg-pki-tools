...
class OSGPKIUtilsTests(unittest.TestCase):
    ...
    def test_alt_name_csr_generation(self):
        ...
        self.assertIsNotNone(
            re.search(r"X509v3 Subject Alternative Name: critical", csr_contents),
            "Subject Alternative Name not marked as 'critical'\n" + csr_contents,
        )
        found_names = {match.group(1) for match in re.finditer(r"DNS:([\w\-\.]+)", csr_contents)}
        expected_names = {alias}
        self.assertEqual(
            found_names, expected_names, f"Did not find expected SAN contents {list(expected_names)}:\n{csr_contents}"
        )  # printed lists are easier to read` than printed sets
        ...
        response = dumps(
            {
                ...
                "detail": " -- ".join(
                    [
                        "Failed to find GridAdmins for specified CSRs/VO",
    ...
    def test_read_config(self):
        ...
        for section, itb in list({"production": False, "ITB": True}.items()):
            self.assertTrue(
                OSGPKIUtils.read_config(itb, config_files=["../osgpkitools/pki-clients.ini"]),
